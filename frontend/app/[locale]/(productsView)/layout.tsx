    ...
    return (
        <ProductFilterProvider>
            <Container overflow={"visible"}>
                <Heading textAlign={"center"} fontSize={{ base: "22px", md: "28px", lg: "32px", "2xl": "38px" }} fontWeight={"extrabold"}>{t(currentPageTitle)}</Heading>
                <Text textAlign={"center"} mt={{ base: "12px", lg: "16px" }} mb={{ base: "26px", md: "40px", xl: "54px", "2xl": "70px" }} color={"gray"} fontSize={{ base: "12px", md: "14px", lg: "16px", "2xl": "18px" }}>{t("productCatalogDescription")}</Text>
                <HStack gap={"32px"} align="start">
                    <Box display={{ md: "none" }} position={"fixed"} right={"0"} top={"40%"} zIndex={"10"}>
                        <SmallScreenFilters />
                    </Box>
                    <Box display={{ base: "none", md: "block" }}>
                        <Filters />
                    </Box>
                    <VStack flex={1} align={"stretch"} gap={{ base: "22px", md: "34px", xl: "48px" }}>
                        <Search />
                        <ProductTabsHeader />
                        {children}
                    </VStack>
