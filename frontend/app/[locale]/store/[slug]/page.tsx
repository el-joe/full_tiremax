...
const page = async ({ params }: props) => {
  ...
  const { data: reviewsData } = await axiosInstance<{
    data: IReview[];
  ...
  return (
    <>
      <HStack
        gap={{ base: "16px", md: "26px", lg: "32px", xl: "40px" }}
        flexWrap={"wrap"}
        align={"start"}
        pb={{ base: "66px", "2xl": "36px" }}
      >
        <ProductImagesPreview product={data.data} />
        <ProductDetails product={data.data} reviews={reviewsData} />
      </HStack>
